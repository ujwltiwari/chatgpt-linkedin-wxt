...
const ModalContent = ({ openModal }: { openModal: () => void }) => {
  const handleClick = () => {
    openModal()
  }
...
export default () => {
  ...
  const injectComponent = () => {
    ...
    console.log('msgBox:', msgBox) // Check if msgBox is found
    if (msgBox) {
      const reactContainer = document.createElement('div')
      reactContainer.id = 'react-injection'
      msgBox.appendChild(reactContainer)
      const root = ReactDOM.createRoot(reactContainer)
      root.render(<ModalContent openModal={openModal} />)
      setComponentMounted(true)
    }
  }

  const removeComponent = () => {
    const reactContainer = document.getElementById('react-injection')
    if (reactContainer) {
  ...
  useEffect(() => {
    const observer = new MutationObserver(() => {
      ...
      if (msgBox) {
        ...
        msgBox.addEventListener('focus', () => {
          if (!componentMounted) {
            setTimeout(() => injectComponent(), 100) // Small delay to avoid interference
          }
        })
        // Blur event listener to remove component
        msgBox.addEventListener('blur', () => {
          if (componentMounted) {
            setTimeout(() => removeComponent(), 100) // Small delay for smooth transition
          }
        })
        observer.disconnect()
      }
    ...
    return () => {
      ...
      if (msgBox) {
        ...
        msgBox.removeEventListener('focus', injectComponent)
        msgBox.removeEventListener('blur', removeComponent)
      }
    }
  }, [componentMounted])
  ...
  const insertMsg = () => {
    ...
    const mgsPlaceholder = document.querySelector(
      '.msg-form__placeholder'
    ) as Element
    msgBox.innerHTML = ''
    mgsPlaceholder.setAttribute('data-placeholder', '')
    msgBox.innerHTML = `Thank you for the opportunity! If you have any more questions or if there's anything else I can help you with, feel free to ask.`
  }

  // return null
  return (
    <div className='mt-[2px]'>
      <Modal showModal={showModal} onClose={closeModal} onClick={insertMsg} />
    </div>
  )
}
